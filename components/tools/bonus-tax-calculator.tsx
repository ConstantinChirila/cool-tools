  ...
  return (
    <>
      <div className="grid gap-6 lg:grid-cols-[5fr_6fr] lg:items-start">
        <BonusInputs input={input} update={update} comparison={comparison} />
        <BonusResults input={input} comparison={comparison} />
      </div>
      <BonusBreakdown input={input} result={comparison.result} />
      <MobileResultBar label="You keep" value={money(comparison.result.takeHome)} />
    </>
  );
}

type Update = <K extends keyof BonusInput>(key: K, value: BonusInput[K]) => void;
...
function BonusInputs({
  ...
  input: BonusInput;
  update: Update;
  comparison: SacrificeComparison;
}) {
  const sections = useSectionState(["sacrifice"]);
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2.5">
          <Section
            ...
            open={sections.isOpen("sacrifice")}
            onToggle={() => sections.toggle("sacrifice")}
          >
            <SliderField
              id="sacrifice"
          ...
          <Section
            ...
            open={sections.isOpen("other")}
            onToggle={() => sections.toggle("other")}
          >
...
function BonusResults({
  input,
  comparison: { result, allCash, allPension, pensionPerPound },
}: {
  input: BonusInput;
  comparison: SacrificeComparison;
}) {
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <SplitBar
            segments={[
              ...
              // What leaves the bonus, so the bar adds up to it: relief at source tops the pot up from outside.
              { name: "Pension", value: result.sacrificed + result.pension, color: "var(--chart-3)" },
            ].filter((s) => s.value > 0)}
            format={(v) => money(v)}
          />
...
function BonusBreakdown({ input, result }: { input: BonusInput; result: BonusResult }) {
